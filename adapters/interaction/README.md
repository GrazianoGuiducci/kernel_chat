...
AI interaction host's persistent/custom instruction surface. It does not
...
A chat is one possible interaction host, not the definition of this carrier. A
...
entry can inhabit other interaction surfaces exposing equivalent capabilities.
...
Another interaction host may use project knowledge, a filesystem,
another repository service, MCP or another persistent source. The receiving
mechanism changes; the kernel relation does not.

`VERSION` identifies this portable interaction-entry contract. Host-adapter
